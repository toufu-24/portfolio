  ...
  useEffect(() => {
    ...
    const drawSparse = (time: number) => {
      ...
      for (const bar of sparseBars) {
        ...
        ctx.fillStyle = `rgba(0, 212, 255, ${bar.height > 0.3 ? 0.15 : 0.04})`
        ctx.fillRect(x, h / 2 - bh / 2, 1.5, bh)
      }
    }

    /* -- Convergence curve (objective value → 0) -- */
    const drawConvergence = (time: number) => {
      ...
      for (let x = 0; x <= rangeX; x++) {
        ...
        const value = Math.exp(-progress * 4) * (1 + Math.sin(progress * 20 + t) * 0.15 * Math.exp(-progress * 3))
        const px = pad + x
      ...
      const dotVal = Math.exp(-dotProgress * 4) * (1 + Math.sin(dotProgress * 20 + t) * 0.15 * Math.exp(-dotProgress * 3))
      ...
      ctx.fillText("f(xₖ)", pad - 2, h * 0.12)
      ctx.fillText("iteration k →", w - pad - 60, h * 0.93)
      ctx.fillText("convergence", w * 0.45, h * 0.93)
    }

    /* -- Graph edges animation -- */
    ...
    const drawGraph = (time: number) => {
      ...
      // Nodes with graph coloring
      for (const n of graphNodes) {
        const pulse = 0.5 + Math.sin(t + n.x * 10) * 0.3
        const colorStr = graphColors[n.color].replace("ALPHA", String(0.15 + pulse * 0.1))
        ctx.beginPath()
      ...
      ctx.fillText("χ(G) ≤ Δ + 1", w * 0.02, h * 0.9)
      ctx.fillText("graph coloring", w * 0.8, h * 0.9)
    }

    const animate = (time: number) => {
      switch (variant) {
        case "sine": drawSine(time); break
        case "sparse": drawSparse(time); break
        case "convergence": drawConvergence(time); break
        case "graph": drawGraph(time); break
      }
